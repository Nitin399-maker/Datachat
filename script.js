...
$tablesContainer.addEventListener("input", (e) => {
  ...
  const $explainPrompt = e.target.closest("#explain-prompt");
  if ($explainPrompt) DB.explainPrompt = $explainPrompt.value;
});
...
async function drawTables() {
  ...
  const query = () => html`
    ...
    <div class="mb-3">
      <label for="explain-prompt" class="form-label fw-bold">explanation system prompt:</label>
      <textarea class="form-control" name="explain-prompt" id="explain-prompt" rows="2">${DB.explainPrompt}</textarea>
    </div>
      <div class="mb-3">
...
5. The SQL must be inside a single code block formatted as single code fence.
    ...
    if (data.length > 0) {
      ...
      const actions = html`
        <div class="row align-items-center g-2">
          <div class="col-auto">
            <button id="explain-button" type="button" class="btn btn-success">
      ...
      render([actions, html`<div id="explanation-container"></div>`, tableHtml], $result);
    } else {
...
$result.addEventListener("click", async (e) => {
  ...
  if ($explainButton && latestQueryResult.length > 0) {
    const $explanationContainer = document.getElementById("explanation-container");
    render(html`<div class="alert alert-info my-3">${loading}</div>`, $explanationContainer);
...
Total rows returned: ${latestQueryResult.length}
    ...
    const explanation = await llm({
      system: DB.explainPrompt,
      user: explanationContent
    });
    render(html`<div class="alert alert-info my-3">${explanation}</div>`, $explanationContainer);
  }
